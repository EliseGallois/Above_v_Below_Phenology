...
# read in data files
hobo <- "users/egallois/soilcores/data/cairngorms_HOBO_pendants"
...
# write as csv
write.csv(clean_cairn, file = "users/egallois/soilcores/data/Cairn_FullHOBO_2022.csv", row.names = FALSE)
...
# Read-in data files
tomst <- "users/egallois/soilcores/data/coastal_mtn_TOMST"
...
#### 4 - Kluane TOMST ####
tomst <- "users/egallois/soilcores/data/kluane_TOMST"
files <- list.files(path = tomst, pattern = "^data_*", full.names = T)
...
#### 4 - Merge data frames ####
# write as csv
write.csv(tomst_kp, file = "users/egallois/soilcores/data/KP_FullTOMST_2022.csv", row.names = FALSE)



#### 5 - Toolik TOMST ####
tomst <- "users/egallois/soilcores/data/toolik_TOMST"
files <- list.files(path = tomst, pattern = "^data_*", full.names = T)
...
#### 4 - Merge data frames ####
# write as csv
write.csv(tomst_bc, file = "users/egallois/soilcores/data/TL_FullTOMST_2022.csv", row.names = FALSE)


#### 6 - Niwot TOMST ####
# read in raw data files
NIW_08 <- read_csv("users/egallois/soilcores/data/niwot_TOMST/sn_08_tenminute.jm.data.csv")
NIW_11 <- read_csv("users/egallois/soilcores/data/niwot_TOMST/sn_11_tenminute.jm.data.csv")
NIW_12 <- read_csv("users/egallois/soilcores/data/niwot_TOMST/sn_12_tenminute.jm.data.csv")
NIW_13 <- read_csv("users/egallois/soilcores/data/niwot_TOMST/sn_13_tenminute.jm.data.csv")
...
# 2021 only until i clean it!±!
cairn_july_surface_temp <- cg_data %>%
  group_by(Date) %>% 
  summarise(mean_temp = mean(Temperature)) %>% 
  subset(Date >= "2021-07-30" & Date <= "2021-07-31")
mean(cairn_july_surface_temp$mean_temp) # 12.4797


kluane_july_surface_temp <- kp_data %>%
  subset(Date >= "2021-07-30" & Date <= "2021-07-31" |
           Date >= "2022-07-01" & Date <= "2022-07-31") %>% 
  filter(Variable %in% "T2: Surface sensor") %>% 
  summarise(mean_temp = mean(Value)) 
  
mean(kluane_july_surface_temp$mean_temp) # 7.805064


bc_july_surface_temp <- bc_data %>%
  subset(Date >= "2021-07-30" & Date <= "2021-07-31" |
           Date >= "2022-07-01" & Date <= "2022-07-31") %>% 
  filter(Variable %in% "T2: Surface sensor") %>% 
  summarise(mean_temp = mean(Value)) 

mean(bc_july_surface_temp$mean_temp) # 3.486911

tl_july_surface_temp <- tl_data %>%
  subset(Date >= "2021-07-30" & Date <= "2021-07-31" |
           Date >= "2022-07-01" & Date <= "2022-07-31") %>% 
  filter(Variable %in% "T2: Surface sensor") %>% 
  summarise(mean_temp = mean(Value)) 

mean(tl_july_surface_temp$mean_temp) # 18.14274


niwot_july_surface_temp <- nw_data %>%
  group_by(Date) %>% 
  summarise(mean_temp = mean(airtemp_avg)) %>% 
  subset(Date >= "2021-07-01" & Date <= "2021-07-31" |
           Date >= "2022-07-01" & Date <= "2022-07-31")

mean(niwot_july_surface_temp$mean_temp, na.rm = TRUE) # 11.76672
...
# save as csvs
write.csv(CG_mean_daily_temp, file = "users/egallois/soilcores/data/CG_mean_daily_temp.csv", row.names = FALSE)
write.csv(BC_mean_daily_temp, file = "users/egallois/soilcores/data/BC_mean_daily_temp.csv", row.names = FALSE)
write.csv(KP_mean_daily_temp, file = "users/egallois/soilcores/data/KP_mean_daily_temp.csv", row.names = FALSE)
write.csv(TL_mean_daily_temp, file = "users/egallois/soilcores/data/TL_mean_daily_temp.csv", row.names = FALSE)
write.csv(NW_mean_daily_temp, file = "users/egallois/soilcores/data/NW_mean_daily_temp.csv", row.names = FALSE)
...
write.csv(full_sites, file = "users/egallois/soilcores/data/all_sites_mean_daily_temp.csv", row.names = FALSE)
